...
class Preprocess_OSA(Preprocess):
    ...
    def Read1DataFile(self, file_name):
        ...
        mat_data = sio.loadmat(file_path)   #加载.mat文件
        resample = 3000
        psg_use = list()
...
if __name__ == '__main__':
    ...
    for i in range(0, len(osa_process.data_file_list)):  # 遍历每个病人的数据和标签文件
        ...
        label = osa_process.Read1LabelFile(label_path)    #读取数据和标签
        ...
        fold_data.append(data.astype(np.float32))  # 将数据转换为 float32 类型
    ...
    np.savez(os.path.join(osa_process.preprocess_config['save_path'], '111.npz'),
             Fold_data=fold_data,
             Fold_label=fold_label,
             Fold_len=fold_len
    )
    print('Saved to', os.path.join(osa_process.preprocess_config['save_path'], '111.npz'))
