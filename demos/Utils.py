...
def AddContext_MultiSub(x, y, Fold_Num, context, i):
    ...
    input:
        x       : [N,V,F];             原始特征数据
        y       : [N,C];               One-hot 标签
        Fold_Num: [kfold];             每个fold的样本数
        context : int;                 上下文窗口大小（必须为奇数）
        i       : int (i-th fold)      当前 fold 的索引
    return:
        x_c     : 添加上下文后的特征数据，形状 [N', context, V, F]
        y_c     : 同步后的标签数据，形状 [N', C]
    '''
    cut = context // 2  # 左右各裁掉多少帧
    fold = Fold_Num.copy()
    fold = np.delete(fold, -1)  # 移除最后一个（防止越界）

    # 计算上下文拼接后需要删除的边界索引
    id_del = np.concatenate([np.cumsum(fold) - i for i in range(1, context)])
    id_del = np.sort(id_del)

    # 添加上下文：每一帧变成一个 [context, V, F] 的片段
    ...
    # 筛选出合法范围内的 id_del
    id_del = id_del[id_del < x_c.shape[0]]

    # 删除边界处跨 fold 的样本
    x_c = np.delete(x_c, id_del, axis=0)
    y_c = np.delete(y_c, id_del, axis=0)
